...
export function fixWorker() {
  ...
  Platform.HostRuntime.HOST_RUNTIME.createWorker = (url: string) => {
    if (url.endsWith("/entrypoints/heap_snapshot_worker/heap_snapshot_worker-entrypoint.js")) {
      return createWorker(new URL("/playground/devtools/heap_snapshot_worker.js", import.meta.url).toString());
    }
    return createWorker(url);
  };
  FormatterWorkerPool.instance({
    forceNew: true,
    entrypointURL: new URL("/playground/devtools/formatter_worker.js", import.meta.url).toString(),
  });
}
