...
import "chrome-devtools-frontend/front_end/panels/profiler/profiler-meta.ts";
...
export function modifyConsoleView() {
  function modifyUI(
    ConsolePanel: typeof import("chrome-devtools-frontend/front_end/panels/console/console.ts").ConsolePanel,
  ) {
    ...
    editor.dispatch({ changes: [{ from: 0, insert: state.code! }] });
  ...
  function addEvaluateModeOption(
    ...
    evaluate.addEventListener("change", () => {
      ...
      Engine262ConnectionTransport.instance?.sendRawMessage?.(
        JSON.stringify({ id: "", method: "Debugger.engine262_setEvaluateMode", params: { mode: evaluate.value } }),
      );
